...
const COLOR_SELECTED_RANGE: Color32 = Color32::from_rgb(255, 236, 204);
...
pub fn show_commit_list(ui: &mut Ui, state: &mut AppState) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        for (idx, commit) in state.commits.iter().enumerate() {
            let is_selected = state.selected_commits.contains(&idx);
            let bg = if is_selected {
                if state.selected_commits.len() == 2 {
                    COLOR_SELECTED_RANGE
                } else {
                    COLOR_SELECTED
                }
            } else {
        ...
        if let Some(idx) = clicked_idx {
            if ctrl_held && !state.selected_commits.is_empty() {
                ...
                state.selected_commits = vec![idx];
            }
